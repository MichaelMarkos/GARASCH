...
namespace NewGarasAPI.Controllers
{
    ...
    public class PurchesRequestController : Controller
    {
        ...
        [HttpGet("GetItemsForCreatePurchaseRequest")]
        public InventoryItemMatrialReleaseInfoResponse GetItemsForCreatePurchaseRequest([FromHeader]long MatrialRequestID)
        {
            ...
            try
            {
                ...
                if (response.Result)
                {
                    var data =  _purchesRequestService.GetItemsForCreatePurchaseRequest(MatrialRequestID, validation.userID);
                    if (!response.Result)
                    {
                        response.Result = false;
                        response.Errors.AddRange(data.Errors);
        ...
        [HttpPost("AddInventoryItemPurchaseRequest")]
        public BaseResponseWithID AddInventoryItemPurchaseRequest(AddInventoryItemMatrialReleaseRequest newData)
        {
            BaseResponseWithID response = new BaseResponseWithID();
            response.Result = true;
            response.Errors = new List<Error>();
            ...
            try
            {
                ...
                if (response.Result)
                {
                    var data = _purchesRequestService.AddInventoryItemPurchaseRequest(newData, validation.userID);
                    if (!response.Result)
                    {
                        response.Result = false;
                        response.Errors.AddRange(data.Errors);

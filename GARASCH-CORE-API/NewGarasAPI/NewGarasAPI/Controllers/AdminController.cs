...
namespace NewGarasAPI.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        [HttpGet("GetInventoryUOM")]        //service Added
        public async Task<GetInventoryUOMResponse> GetInventoryUOM()
        {
            ...
            try
            {
                ...
                if (response.Result)
                {
                    ...
                    //    //var CheckUserDB = _Context.proc_UserLoadAll().Where(x => (x.Email.Trim() == login.Email.Trim()) && x.Password.ToLower().Trim() == Encrypt_Decrypt.Encrypt(login.Password.Trim(), key).ToLower().Trim()).FirstOrDefault();
                    //    var GetInventoryUOMDB = await _Context.InventoryUoms.ToListAsync();
                    ...
                    //            InventoryUOMResponse.Name = GetInventoryUOMDBJ.Name;

                    //            InventoryUOMResponse.ShortName = GetInventoryUOMDBJ.ShortName;
                    ...
                    var DailyReportThrough =await _adminService.GetInventoryUOM();
                    if (!DailyReportThrough.Result)
                    {
                        response.Result = false;
                        response.Errors.AddRange(DailyReportThrough.Errors);
        ...
        [HttpPost("AddEditInventoryUOM")]       //service Added
        public ActionResult<BaseResponseWithID> AddEditInventoryUOM(InventoryUOMData request)
        {
            BaseResponseWithID Response = new BaseResponseWithID();
            Response.Result = true;
            Response.Errors = new List<Error>();
            try
            {
                ...
                if (Response.Result)
                {
                    ...
                    //    if (request.ID != null && request.ID != 0)
                    //    {
                    //        var InventoryUOMDB = _Context.InventoryUoms.Find(request.ID);
                    ...
                    //            InventoryUOMDB.ModifiedBy = validation.userID;
                    //            InventoryUOMDB.ModifiedDate = DateTime.Now;

                    //            var InventoryUOMUpdate = _Context.SaveChanges();
                    ...
                    //            ModifiedDate = DateTime.Now,
                    //        };
                    //        _Context.InventoryUoms.Add(InventoryUom);

                    //        var InventoryUOMInsert = _Context.SaveChanges();
                    ...
                    var InventoryUOM = _adminService.AddEditInventoryUOM(request, validation.userID);
                    if (!InventoryUOM.Result)
                    {
                        Response.Result = false;
                        Response.Errors.AddRange(InventoryUOM.Errors);
                        return Response;
                    }
                    Response = InventoryUOM;
                }
        ...
        [HttpPost("AddInventoryStore")]        //Service Added
        public async Task<BaseResponseWithId<int>> AddInventoryStore(AddInventoryStoreData request)
        {
            ...
            try
            {
                ...
                {
                    _adminService.Validation = validation;
                    Response = await _adminService.AddInventoryStore(request);
                }

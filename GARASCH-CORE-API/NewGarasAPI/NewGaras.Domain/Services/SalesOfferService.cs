...
namespace NewGaras.Domain.Services
{
    public class SalesOfferService : ISalesOfferService
    {
        ...
        public InventoryItemFullInfo GetInventoryItemFullInfo(long InventoryItemId)
        {
            ...
            var InventoryItemInfoObj = new InventoryItemFullInfo();

            var InventoryItemObjDB = _unitOfWork.InventoryItems.GetById(InventoryItemId);
            if (InventoryItemObjDB != null)
            {
                ...
                InventoryItemInfoObj.Category = _unitOfWork.InventoryItemCategories.GetById(InventoryItemObjDB.InventoryItemCategoryId)?.Name;
                InventoryItemInfoObj.PurchasingUnit = Common.GetInventoryUOM(InventoryItemObjDB.PurchasingUomid, _Context);
                InventoryItemInfoObj.RequestionUnit = Common.GetInventoryUOM(InventoryItemObjDB.RequstionUomid, _Context);
                ...
                if (CalcType == 1)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = "Average";
                    Amount = InventoryItemObjDB.AverageUnitPrice;
                }
                else if (CalcType == 2)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = "Max";
                    Amount = InventoryItemObjDB.MaxUnitPrice;
                }
                else if (CalcType == 3)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = "Last";
                    Amount = InventoryItemObjDB.LastUnitPrice;
                }
                else if (CalcType == 4)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = "Custom";
                    Amount = InventoryItemObjDB.CustomeUnitPrice;
                }
                else if (CalcType == 5)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName1;
                    Amount = InventoryItemObjDB.CostAmount1 ?? 0;
                }
                else if (CalcType == 6)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName2;
                    Amount = InventoryItemObjDB.CostAmount2 ?? 0;
                }
                else if (CalcType == 7)
                {
                    InventoryItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName3;
                    Amount = InventoryItemObjDB.CostAmount3 ?? 0;
                }
                InventoryItemInfoObj.Amount = Amount;

                List<SelectDDL> BOMsList = new List<SelectDDL>();
                var BOMsListDB = _unitOfWork.Boms.FindAll(a => a.ProductId == InventoryItemObjDB.Id && a.MainBomid == null).OrderByDescending(a => a.CreationDate).ToList();
                foreach (var bom in BOMsListDB)
                {
                    SelectDDL BOMObj = new SelectDDL
                    {
                        ID = bom.Id,
                        Name = bom.Name + " / " + bom.Serial
                    };
                    BOMsList.Add(BOMObj);
                }
                InventoryItemInfoObj.BOMsList = BOMsList;

                List<GetInventoryItemPrice> PricesList = new List<GetInventoryItemPrice>();
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 1,
                    PriceAmount = InventoryItemObjDB.AverageUnitPrice,
                    PriceName = "Average"
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 2,
                    PriceAmount = InventoryItemObjDB.MaxUnitPrice,
                    PriceName = "Max"
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 3,
                    PriceAmount = InventoryItemObjDB.LastUnitPrice,
                    PriceName = "Last"
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 4,
                    PriceAmount = InventoryItemObjDB.CustomeUnitPrice,
                    PriceName = "Custom"
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 5,
                    PriceAmount = InventoryItemObjDB.CostAmount1 ?? 0,
                    PriceName = InventoryItemObjDB.CostName1
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 6,
                    PriceAmount = InventoryItemObjDB.CostAmount2 ?? 0,
                    PriceName = InventoryItemObjDB.CostName2
                });
                PricesList.Add(new GetInventoryItemPrice
                {
                    Id = 7,
                    PriceAmount = InventoryItemObjDB.CostAmount3 ?? 0,
                    PriceName = InventoryItemObjDB.CostName3
                });
                InventoryItemInfoObj.PricesList = PricesList;
            }

            var InventoryStoreItemListDB = _unitOfWork.VInventoryStoreItems.FindAll(a => a.InventoryItemId == InventoryItemId && a.StoreActive == true).GroupBy(a => new { StoreId = a.InventoryStoreId, StoreName = a.InventoryStoreName }).ToList();
            if (InventoryStoreItemListDB.Count() > 0)
            {
                List<StoreStockModel> StoresStock = new List<StoreStockModel>();
                decimal totalStoresBalance = 0;
                foreach (var store in InventoryStoreItemListDB)
                {
                    var StoreBalance = store.Sum(a => a.Balance);
                    totalStoresBalance += StoreBalance;
                    var StoreStock = new StoreStockModel
                    {
                        InventoryStoreId = store.Key.StoreId,
                        InventoryStoreName = store.Key.StoreName,
                        Balance = StoreBalance
                    };
                    StoresStock.Add(StoreStock);
                }
                InventoryItemInfoObj.StoresStockList = StoresStock;
                InventoryItemInfoObj.TotalBalance = totalStoresBalance;
            }

            return InventoryItemInfoObj;
        }
        ...
        public GetOfferInventoryItemResponse GetOfferInventoryItemInfo(long InventoryItemId)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    if (Response.Result)
                    {
                        var InventoryItemInfoObj = GetInventoryItemFullInfo(InventoryItemId);
                        Response.InventoryItem = InventoryItemInfoObj;
                    }
                }
                return Response;
            }
        ...
        public GetBOMInventoryItemsResponse GetBOMInventoryItemsInfo(long BOMId)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    if (Response.Result)
                    {
                        List<BOMPartitionItemInfo> BOMPartitionItemsListResponse = new List<BOMPartitionItemInfo>();
                        var BOMPartitionsIds = _unitOfWork.Bompartitions.FindAll(a => a.Bomid == BOMId && a.Active == true).Select(a => a.Id).ToList();
                        var BOMPartitionsItemsDB = _unitOfWork.VBompartitionItemsInventoryItems.FindAll(a => BOMPartitionsIds.Contains(a.BompartitionId) && a.BompartitionItemsActive == true).ToList();
                        decimal BOMTotalCurrentPrice = 0;
                        decimal BOMTotalAvgPrice = 0;
                        decimal BOMTotalLastPrice = 0;
                        decimal BOMTotalMaxPrice = 0;
                        decimal BOMTotalCustomPrice = 0;
                        decimal BOMTotalPrice1 = 0;
                        decimal BOMTotalPrice2 = 0;
                        decimal BOMTotalPrice3 = 0;

                        foreach (var item in BOMPartitionsItemsDB)
                        {
                            BOMPartitionItemInfo BOMPartitionItemInfoObj = new BOMPartitionItemInfo()
                            {
                                ID = item.ItemId,
                                BOMPartitionItemId = item.BompartitionItemId,
                                BOMPartitionID = item.BompartitionId,
                                StoreCategoryId = item.StoreCategoryId,
                                ItemCategoryID = item.ItemCategoryId,
                                ItemOrder = item.ItemOrder,
                                RequiredQty = item.RequiredQty,
                                ItemQtyPrice = item.ItemQtyPrice,
                                BOMPartitionItemPriceType = item.ItemPriceType,
                                IsAlternative = item.IsAlternative,
                                AlternativeItem = item.AlternativeItem,
                                ActiveToUse = item.ActiveToUse,
                                BOMPartitionItemDescription = item.BompartitionItemsDescription,
                                ItemName = Common.getInventoryItemName(item.ItemId, _Context),
                                Category = _unitOfWork.InventoryItemCategories.GetById((int)item.ItemCategoryId)?.Name,
                                ItemCode = item.InventoryItemCode,
                                PartNumber = item.PartNo,
                                PurchasingUnit = _unitOfWork.InventoryUoms.GetById((int)item.PurchasingUomid)?.ShortName,
                                RequestionUnit = _unitOfWork.InventoryUoms.GetById((int)item.RequstionUomid)?.ShortName,
                                MinBalance = (decimal)(item.MinBalance1??0),
                                MaxBlanace = (decimal)(item.MaxBalance1??0),
                                ConvertRateFromPurchasingToRequestionUnit = item.ExchangeFactor1 != null ? (decimal)item.ExchangeFactor1 : 0
                            };

                            string itemUOR = "";
                            var InventoryStoreItemListDB = _unitOfWork.VInventoryStoreItems.FindAll(a => a.InventoryItemId == item.ItemId && a.StoreActive == true).GroupBy(a => new { StoreId = a.InventoryStoreId, StoreName = a.InventoryStoreName }).ToList();
                            if (InventoryStoreItemListDB.Count() > 0)
                            {
                                List<StoreStockModel> StoresStock = new List<StoreStockModel>();
                                decimal totalStoresBalance = 0;
                                ...
                                BOMPartitionItemInfoObj.StoresStockList = StoresStock;
                                BOMPartitionItemInfoObj.TotalBalance = totalStoresBalance;

                                itemUOR = InventoryStoreItemListDB.FirstOrDefault().FirstOrDefault().RequestionUomshortName;
                                itemUOR = Regex.Replace(itemUOR, "\t", "");
                                itemUOR = Regex.Replace(itemUOR, " ", "");

                                decimal availableQty = item.RequiredQty != 0 ? (totalStoresBalance / item.RequiredQty) * 100 : 0;
                                string availableQtyString = String.Format("{0:0.0}", availableQty + "%");
                                if (availableQty < 100)
                                {
                                    BOMPartitionItemInfoObj.AvailableQty = totalStoresBalance + "(" + itemUOR + ")-" + availableQtyString;
                                }
                                else
                                {
                                    BOMPartitionItemInfoObj.AvailableQty = totalStoresBalance + "(" + itemUOR + ")-" + 100 + "%";
                                }
                            }
                            var InventoryItemObjDB = _unitOfWork.VInventoryItems.FindAll(a => a.Id == item.ItemId).FirstOrDefault();
                            List<GetInventoryItemPrice> PricesList = new List<GetInventoryItemPrice>();
                            PricesList.Add(new GetInventoryItemPrice
                            {
                                Id = 1,
                            ...
                            BOMTotalAvgPrice += InventoryItemObjDB.AverageUnitPrice;
                            ...
                            BOMTotalMaxPrice += InventoryItemObjDB.MaxUnitPrice;
                            ...
                            BOMTotalLastPrice += InventoryItemObjDB.LastUnitPrice;
                            ...
                            BOMTotalCustomPrice += InventoryItemObjDB.CustomeUnitPrice;

                            PricesList.Add(new GetInventoryItemPrice
                            {
                                Id = 5,
                                PriceAmount = InventoryItemObjDB.CostAmount1 ?? 0,
                                PriceName = InventoryItemObjDB.CostName1 ?? "Price1"
                            });
                            BOMTotalPrice1 += (InventoryItemObjDB.CostAmount1 ?? 0);

                            PricesList.Add(new GetInventoryItemPrice
                            {
                                Id = 6,
                                PriceAmount = InventoryItemObjDB.CostAmount2 ?? 0,
                                PriceName = InventoryItemObjDB.CostName2 ?? "Price2"
                            });
                            BOMTotalPrice2 += (InventoryItemObjDB.CostAmount2 ?? 0);

                            PricesList.Add(new GetInventoryItemPrice
                            {
                                Id = 7,
                                PriceAmount = InventoryItemObjDB.CostAmount3 ?? 0,
                                PriceName = InventoryItemObjDB.CostName3 ?? "Price3"
                            });
                            BOMTotalPrice3 += (InventoryItemObjDB.CostAmount3 ?? 0);

                            BOMPartitionItemInfoObj.PricesList = PricesList;

                            decimal Amount = 0;
                            string CalcType = item.ItemPriceType;
                            if (CalcType == "Avr")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = "Average";
                                Amount = InventoryItemObjDB.AverageUnitPrice;
                            }
                            else if (CalcType == "Max")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = "Max";
                                Amount = InventoryItemObjDB.MaxUnitPrice;
                            }
                            else if (CalcType == "Last")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = "Last";
                                Amount = InventoryItemObjDB.LastUnitPrice;
                            }
                            else if (CalcType == "Custome")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = "Custom";
                                Amount = InventoryItemObjDB.CustomeUnitPrice;
                            }
                            else if (CalcType == "CostName1")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName1 ?? "Price1";
                                Amount = InventoryItemObjDB.CostAmount1 ?? 0;
                            }
                            else if (CalcType == "CostName2")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName2 ?? "Price2";
                                Amount = InventoryItemObjDB.CostAmount2 ?? 0;
                            }
                            else if (CalcType == "CostName3")
                            {
                                BOMPartitionItemInfoObj.PriceCalculationMethod = InventoryItemObjDB.CostName3 ?? "Price3";
                                Amount = InventoryItemObjDB.CostAmount3 ?? 0;
                            }
                            BOMPartitionItemInfoObj.Amount = Amount;
                            BOMTotalCurrentPrice += Amount;
                            BOMPartitionItemsListResponse.Add(BOMPartitionItemInfoObj);
                        }

                        List<BomPrices> bomPrices = new List<BomPrices>();
                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalAvgPrice,
                            PriceName = "Average"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalMaxPrice,
                            PriceName = "Max"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalLastPrice,
                            PriceName = "Last"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalCustomPrice,
                            PriceName = "Custom"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalPrice1,
                            PriceName = "Price1"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalPrice2,
                            PriceName = "Price2"
                        });

                        bomPrices.Add(new BomPrices
                        {
                            PriceAmount = BOMTotalPrice3,
                            PriceName = "Price3"
                        });

                        Response.BomPricesList = bomPrices;
                        Response.BOMCurrentPrice = BOMTotalCurrentPrice;
                        Response.BomInventoryItemList = BOMPartitionItemsListResponse;
                    }
                }
                return Response;
            }
        ...
        public GetSalesOfferItemSellingHistoryResponse GetSalesOfferItemSellingHistory(long InventoryItemId)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    ...
                    var SellingProductsDbList = _unitOfWork.VSalesOfferProductSalesOffers.FindAll(a => a.Status == "Closed" && a.InventoryItemId == InventoryItemId && a.Active == true).OrderByDescending(a => a.ClientApprovalDate).ToList();
                    var SalesOfferItemHistoryList = SellingProductsDbList.Select(item => new SalesOfferItemSalesDetails
                    {
                        OfferId = item.OfferId,
                        InventoryItemId = item.InventoryItemId,
                        InventoryItemName = item.InventoryItemId != null ? Common.getInventoryItemName((long)item.InventoryItemId, _Context) : null,
                        ClientId = item.ClientId,
                        ClientName = item.ClientId != null ? Common.GetClientName((long)item.ClientId, _Context) : null,
                        SoldDate = item.ClientApprovalDate == null ? "" : item.ClientApprovalDate.ToString(),
                        ItemPrice = item.ItemPrice != null ? (decimal)item.ItemPrice : 0
                    }).ToList();
                    Response.SalesOfferItemsSalesDetailsList = SalesOfferItemHistoryList;
        ...
        public GetOfferInventoryItemsListResponse GetOfferInventoryItemsList(GetOfferInventoryItemsListFilters filters)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    var InventoryItemsListQuery = _unitOfWork.VInventoryItems.FindAllQueryable(a => true);
                    ...
                    {
                        InventoryItemsListQuery = InventoryItemsListQuery.Where(a => a.Active == false);
                    }
                    else
                    {
                        InventoryItemsListQuery = InventoryItemsListQuery.Where(a => a.Active == true);
                    }
                    ...
                    if (filters.StoreId != 0)
                    {
                        var InventoryStoreItemsIds = _unitOfWork.VInventoryStoreItems.FindAll(a => a.InventoryStoreId == filters.StoreId && a.StoreActive == true).Select(a => a.InventoryItemId).Distinct().ToList();
                    ...
                    var InventoryItemsListResponse = new List<InventoryItemInfo>();
                    InventoryItemsListResponse = InventoryItemsList.Select(InventoryItemObjDB => new InventoryItemInfo
                    {
                        ID = InventoryItemObjDB.Id,
                        ItemName = InventoryItemObjDB.Name,
                        ItemCode = InventoryItemObjDB.Code,
                        CommericalName = InventoryItemObjDB.CommercialName,
                        MarketName = InventoryItemObjDB.MarketName,
                        Type = InventoryItemObjDB.Exported,
                        Description = InventoryItemObjDB.Description,
                        Details = InventoryItemObjDB.Details,
                        Category = InventoryItemObjDB.CategoryName,
                        CategoryId = InventoryItemObjDB.InventoryItemCategoryId,
                        PurchasingUnit = InventoryItemObjDB.PurchasingUomshortName,
                        RequestionUnit = InventoryItemObjDB.RequestionUomshortName,

                        MinBalance = InventoryItemObjDB.MinBalance != null ? (decimal)InventoryItemObjDB.MinBalance : 0,
                        MaxBlanace = InventoryItemObjDB.MaxBalance != null ? (decimal)InventoryItemObjDB.MaxBalance : 0,
                        ConvertRateFromPurchasingToRequestionUnit = InventoryItemObjDB.ExchangeFactor != null ? (decimal)InventoryItemObjDB.ExchangeFactor : 0,
                        Active = InventoryItemObjDB.Active,
                        Amount =
                                (
                                    InventoryItemObjDB.CalculationType == 1 ? InventoryItemObjDB.AverageUnitPrice :
                                    InventoryItemObjDB.CalculationType == 2 ? InventoryItemObjDB.MaxUnitPrice :
                                    InventoryItemObjDB.CalculationType == 3 ? InventoryItemObjDB.LastUnitPrice :
                                    InventoryItemObjDB.CalculationType == 4 ? InventoryItemObjDB.CustomeUnitPrice :
                                    InventoryItemObjDB.CalculationType == 5 ? InventoryItemObjDB.CostAmount1 :
                                    InventoryItemObjDB.CalculationType == 6 ? InventoryItemObjDB.CostAmount2 :
                                    InventoryItemObjDB.CalculationType == 7 ? InventoryItemObjDB.CostAmount3 : 0
                                ) ?? 0,
                        PriceCalculationMethod =
                                (
                                    InventoryItemObjDB.CalculationType == 1 ? "Average" :
                                    InventoryItemObjDB.CalculationType == 2 ? "Max" :
                                    InventoryItemObjDB.CalculationType == 3 ? "Last" :
                                    InventoryItemObjDB.CalculationType == 4 ? "Custom" :
                                    InventoryItemObjDB.CalculationType == 5 ? InventoryItemObjDB.CostName1 :
                                    InventoryItemObjDB.CalculationType == 6 ? InventoryItemObjDB.CostName2 :
                                    InventoryItemObjDB.CalculationType == 7 ? InventoryItemObjDB.CostName3 : null
                                ),
                        ItemImage =
                        (
                            (_unitOfWork.InventoryItems.GetById(InventoryItemObjDB.Id)?.ImageUrl != null && InventoryItemObjDB.HasImage == true && InventoryItemObjDB.Id != 0) ?
                            Globals.baseURL + _unitOfWork.InventoryItems.GetById(InventoryItemObjDB.Id)?.ImageUrl : null
                        )
                    }).ToList();
                    //foreach (var InventoryItemObjDB in InventoryItemsList)
                    //{
                    //    if (InventoryItemObjDB != null)
                    //    {
                    ...
                    //        InventoryItemInfoObj.Description = InventoryItemObjDB.Description;
                    //        InventoryItemInfoObj.Details = InventoryItemObjDB.Details;
                    //        InventoryItemInfoObj.Category = Common.GetInventoryItemCategory(InventoryItemObjDB.InventoryItemCategoryID);
                    //        InventoryItemInfoObj.PurchasingUnit = Common.GetInventoryUOM(InventoryItemObjDB.PurchasingUOMID);
                    //        InventoryItemInfoObj.RequestionUnit = Common.GetInventoryUOM(InventoryItemObjDB.RequstionUOMID);

                    //        InventoryItemInfoObj.MinBalance = InventoryItemObjDB.MinBalance != null ? (decimal)InventoryItemObjDB.MinBalance : 0;
                    //        InventoryItemInfoObj.MaxBlanace = InventoryItemObjDB.MaxBalance != null ? (decimal)InventoryItemObjDB.MaxBalance : 0;
                    //        InventoryItemInfoObj.ConvertRateFromPurchasingToRequestionUnit = InventoryItemObjDB.ExchangeFactor != null ? (decimal)InventoryItemObjDB.ExchangeFactor : 0;
                    //        InventoryItemInfoObj.Active = InventoryItemObjDB.Active;
                    //        if (InventoryItemObjDB.Image != null && InventoryItemObjDB.HasImage == true)
                    //        {
                    //            InventoryItemInfoObj.ItemImage = baseURL + "/ShowImage.ashx?ImageID=" + HttpUtility.UrlEncode(Encrypt_Decrypt.Encrypt(InventoryItemId.ToString(), key)) + "&type=item&CompName=" + headers["CompanyName"].ToString().ToLower();
                    //        }
                    ...
                    //        InventoryItemsListResponse.Add(InventoryItemInfoObj);
                    //    }
                    //}
        ...
        public BaseResponseWithId<long> TargetNextYearDetails(TargetNextYearDetailsResponse Request, long creator)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    ...
                    decimal SalesTargetValue = 0;

                    if (Request.SalesTarget != null)
                    {

                        if (Request.SalesTarget.Year == 0)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please Insert a Valid Year (Mandatory)";
                            Response.Errors.Add(error);
                            return Response;
                        }
                        if (Request.SalesTarget.Target == 0)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please Insert a Target (Mandatory)";
                            Response.Errors.Add(error);
                            return Response;
                        }
                        else
                        {
                            SalesTargetValue = Request.SalesTarget.Target;
                        }
                    ...
                    else
                    {
                        ...
                        Error error = new Error();
                        error.ErrorCode = "Err-P12";
                        error.ErrorMSG = "Please Insert The Sales Target";
                        Response.Errors.Add(error);
                        return Response;
                    }


                    decimal SummAmountBranchs = 0;
                    float SummPercentageBranchs = 0;
                    if (Request.SalesTargetBranchList.Count > 0 && Request.SalesTargetBranchList != null)
                    {



                        foreach (var item in Request.SalesTargetBranchList)
                        {
                            if (item.TargetID == 0)
                            {
                                Response.Result = false;
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a Valid TargetID";
                                Response.Errors.Add(error);
                                return Response;
                            }
                            if (item.BranchID == 0)
                            {
                                ...
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a BranchID (Mandatory)";
                                Response.Errors.Add(error);
                                return Response;
                            }

                            SummAmountBranchs = SummAmountBranchs + item.Amount;
                            SummPercentageBranchs = SummPercentageBranchs + item.Percentage;
                            ...
                            if (item.Amount == 0)
                            {
                                ...
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a Amount (Mandatory)";
                                Response.Errors.Add(error);
                                return Response;
                            }
                            ...
                            if (item.Percentage == 0)
                            {
                                ...
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a Percentage (Mandatory)";
                                Response.Errors.Add(error);
                                return Response;
                            }
                            if (item.Percentage >= 101)
                            {
                                Response.Result = false;
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a Valid Branch Percentage";
                                Response.Errors.Add(error);
                                return Response;
                            }
                        ...
                        if (SalesTargetValue != SummAmountBranchs)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please Insert a Valid Amount per Branch";
                            Response.Errors.Add(error);
                            return Response;
                        }
                        if (SummPercentageBranchs != 100)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please The percentage must be 100%";
                            Response.Errors.Add(error);
                            return Response;
                        }
                    ...
                    else
                    {
                        ...
                        Error error = new Error();
                        error.ErrorCode = "Err-P12";
                        error.ErrorMSG = "Please Insert The target branch list";
                        Response.Errors.Add(error);
                        return Response;
                    }
                    ...
                    if (Request.SalesTargetProductList != null && Request.SalesTargetProductList.Count > 0)
                    {
                        decimal SummAmountProducts = 0;
                        float SummPercentageProducts = 0;
                        foreach (var item in Request.SalesTargetProductList)
                        {
                            if (item.TargetID == 0)
                            {
                                Response.Result = false;
                            ...
                            SummAmountProducts = SummAmountProducts + item.Amount;
                            SummPercentageProducts = SummPercentageProducts + item.Percentage;
                            ...
                            if (item.Percentage >= 101)
                            {
                                ...
                                Error error = new Error();
                                error.ErrorCode = "Err-P12";
                                error.ErrorMSG = "Please Insert a valid product Percentage ";
                                Response.Errors.Add(error);
                                return Response;
                            }
                        ...
                        if (SummAmountProducts != SummAmountBranchs)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please Insert a Valid Amount per Product";
                            Response.Errors.Add(error);
                            return Response;
                        }
                        if (SummPercentageProducts != SummPercentageBranchs)
                        {
                            Response.Result = false;
                            Error error = new Error();
                            error.ErrorCode = "Err-P12";
                            error.ErrorMSG = "Please the percentage per product must be valid";
                            Response.Errors.Add(error);
                            return Response;
                        }
                    ...
                    if (Response.Result)
                    {
                        var SalesTargetDB = _unitOfWork.SalesTargets.GetById((int)Request.SalesTarget.ID);
                        if (Request.SalesTarget.ID != null && Request.SalesTarget.ID != 0 && SalesTargetDB != null)
                        {
                            //if (SalesTargetDB != null)
                            //{
                            // Update
                            SalesTargetDB.Year = Request.SalesTarget.Year;
                            SalesTargetDB.Target = Request.SalesTarget.Target;
                            SalesTargetDB.Active = true;
                            SalesTargetDB.CurrencyId = 1;
                            SalesTargetDB.Modified = creator;
                            SalesTargetDB.ModifiedDate = DateTime.Now;
                            _unitOfWork.SalesTargets.Update(SalesTargetDB);

                            var SalesTargetUpdate = _unitOfWork.Complete();
                            if (SalesTargetUpdate > 0)
                            {
                                Response.ID = SalesTargetDB.Id;
                            }
                            else
                            {
                                Response.Result = false;
                                Error error = new Error();
                                error.ErrorCode = "Err25";
                                error.ErrorMSG = "Faild To Update this Sales Target  !!";
                                Response.Errors.Add(error);
                            }
                        }
                        else
                        {
                            ...
                            {
                                Year = Request.SalesTarget.Year,
                                FromDate = new DateTime(Request.SalesTarget.Year, 1, 1),
                                ToDate = new DateTime(Request.SalesTarget.Year, 12, 31),
                                Target = Request.SalesTarget.Target,
                                CreatedBy = creator,
                                CreationDate = DateTime.Now,
                                Active = true,
                                CurrencyId = 1,
                            ...
                            _unitOfWork.SalesTargets.Add(target);
                            var SalesTargetInsert = _unitOfWork.Complete();

                            if (SalesTargetInsert > 0)
                            {
                            ...
                            else
                            {
                                ...
                                Error error = new Error();
                                error.ErrorCode = "Err25";
                                error.ErrorMSG = "Faild To Insert this Sales Target!!";
                                Response.Errors.Add(error);
                            }
                        }

                        var brancheTargets = _unitOfWork.SalesBranchTargets.FindAll(a => Request.SalesTargetBranchList.Select(x => x.ID).Contains(a.Id)).ToList();
                        foreach (var item in Request.SalesTargetBranchList)
                        {
                            if (item.ID != null && item.ID != 0)
                            {
                                var SalesBranchTargetDB = brancheTargets.Where(a => a.Id == item.ID).FirstOrDefault();
                                if (SalesBranchTargetDB != null)
                                {
                                    SalesBranchTargetDB.TargetId = item.TargetID;
                                    SalesBranchTargetDB.BranchId = item.BranchID;
                                    SalesBranchTargetDB.Percentage = item.Percentage;
                                    SalesBranchTargetDB.Amount = item.Amount;
                                    SalesBranchTargetDB.Active = true;
                                    SalesBranchTargetDB.CurrencyId = 1;
                                    SalesBranchTargetDB.ModifiedBy = creator;
                                    SalesBranchTargetDB.Modified = DateTime.Now;
                                    _unitOfWork.SalesBranchTargets.Update(SalesBranchTargetDB);

                                    var SalesBranchTargetUpdate = _unitOfWork.Complete();
                                    if (SalesBranchTargetUpdate > 0)
                                    {
                                        Response.ID = SalesBranchTargetDB.Id;
                                    }
                                    else
                                    {
                                        Response.Result = false;
                                        Error error = new Error();
                                        error.ErrorCode = "Err25";
                                        error.ErrorMSG = "Faild To Update this Sales Branch Target  !!";
                                        Response.Errors.Add(error);
                                    }
                                }
                            }
                            ...
                            {
                                var branchTarget = new SalesBranchTarget()
                                {
                                ...
                                _unitOfWork.SalesBranchTargets.Add(branchTarget);
                                var SalesTargetBranchInsert = _unitOfWork.Complete();


                                if (SalesTargetBranchInsert > 0)
                                {
                                    Response.ID = branchTarget.Id;
                                }
                                else
                                {
                                    Response.Result = false;
                        ...
                        if (Request.SalesTargetProductList != null && Request.SalesTargetProductList.Count > 0)
                        {
                            var productTargets = _unitOfWork.SalesBranchProductTargets.FindAll(a => Request.SalesTargetProductList.Select(x => x.ID).Contains(a.Id)).ToList();
                            foreach (var item in Request.SalesTargetProductList)
                            {

                                if (item.ID != null && item.ID != 0)
                                {
                                    var SalesProductTargetDB = productTargets.Where(a => a.Id == item.ID).FirstOrDefault();
                                    if (SalesProductTargetDB != null)
                                    {
                                        // Update
                                        SalesProductTargetDB.TargetId = item.TargetID;
                                        SalesProductTargetDB.BranchId = item.BranchID;
                                        SalesProductTargetDB.ProductId = item.ProductID;
                                        SalesProductTargetDB.Percentage = item.Percentage;
                                        SalesProductTargetDB.Amount = item.Amount;
                                        SalesProductTargetDB.Active = true;
                                        SalesProductTargetDB.CurrencyId = 1;
                                        SalesProductTargetDB.ModifiedBy = creator;
                                        SalesProductTargetDB.Modified = DateTime.Now;
                                        _unitOfWork.SalesBranchProductTargets.Update(SalesProductTargetDB);
                                        var SalesProductTargetUpdate = _unitOfWork.Complete();
                                        if (SalesProductTargetUpdate > 0)
                                        {
                                            Response.ID = SalesProductTargetDB.Id;
                                        }
                                        else
                                        {
                                            Response.Result = false;
                                            Error error = new Error();
                                            error.ErrorCode = "Err25";
                                            error.ErrorMSG = "Faild To Update this Sales Branch product Target  !!";
                                            Response.Errors.Add(error);
                                        }
                                    }
                                }
                                ...
                                {
                                    var productTarget = new SalesBranchProductTarget()
                                    {
                                        TargetId = item.TargetID,
                                        BranchId = item.BranchID,
                                    ...
                                    _unitOfWork.SalesBranchProductTargets.Add(productTarget);

                                    var SalesBranchProductTargetInsert = _unitOfWork.Complete();

                                    if (SalesBranchProductTargetInsert > 0)
                                    {
                                        Response.ID = productTarget.Id;
                                    }
                                    else
                                    {
                                        Response.Result = false;
        ...
        public async Task<SalesOfferAchievedTargetResponse> SalesOfferAchievedTarget(SalesOfferAchievedTargetFilters filters)
        {
            ...
            try
            {
                if (Response.Result)
                {
                    ...
                    var SalesOfferDBQuery = _unitOfWork.SalesOffers.FindAllQueryable(a => a.Active == true, includes: new[] { "SalesOfferProducts.SalesOfferProductTaxes", "SalesOfferInvoiceTaxes" });
                    ...
                    if (!string.IsNullOrWhiteSpace(filters.OfferStatus))
                    {
                        SalesOfferDBQuery = SalesOfferDBQuery.Where(x => x.Status.ToLower().Trim() == filters.OfferStatus.ToLower().Trim());
                    }
                    if (DateFilter)
                    {
                        if (filters.OfferStatus.ToLower().Trim() == "closed" || filters.OfferStatus.ToLower().Trim() == "rejected")
                        {
                            SalesOfferDBQuery = SalesOfferDBQuery.Where(a => a.ClientApprovalDate >= StartDate && a.ClientApprovalDate <= EndDate).AsQueryable();
                        }
                        else
                    ...
                    decimal? TotalOfferAmount = await SalesOfferDBQuery.SumAsync(x => x.OfferAmount);
                    decimal? TotalFinalOfferPrice = await SalesOfferDBQuery.SumAsync(x => x.FinalOfferPrice);
                    /*decimal? TotalFinalOfferPriceWithoutTax = await SalesOfferDBQuery.SumAsync(SO => SO.FinalOfferPrice??0 -
                    (SO.SalesOfferProducts.SelectMany(x => x.SalesOfferProductTaxes.Select(y => y.Value ?? 0).ToList()).DefaultIfEmpty(0).Sum()) +
                            (SO.SalesOfferInvoiceTaxes.Where(x => x.Active == true).Select(x => x.TaxValue).DefaultIfEmpty(0).Sum())
                            );*/
                    var salesOffers = await SalesOfferDBQuery.ToListAsync();

                    decimal? totalFinalOfferPriceWithoutTax = salesOffers.Sum(SO =>
                        (SO.FinalOfferPrice ?? 0) -
                        (SO.SalesOfferProducts.SelectMany(x => x.SalesOfferProductTaxes.Select(y => y.Value ?? 0)).DefaultIfEmpty(0).Sum()) +
                        (SO.SalesOfferInvoiceTaxes.Where(x => x.Active == true).Select(x => x.TaxValue).DefaultIfEmpty(0).Sum())
                    );
                    int? CountOfSalesOffer = await SalesOfferDBQuery.CountAsync();
                    Response.TotalOfferAmount = TotalOfferAmount;
                    Response.TotalFinalOfferPriceWithoutTax = totalFinalOfferPriceWithoutTax;
                    Response.TotalFinalOfferPrice = TotalFinalOfferPrice;
                    Response.CountOfSalesOffer = CountOfSalesOffer;
                    Response.Result = true;
                    #endregion
